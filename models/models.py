...
class paymentAccount(models.Model):
    ...
    account_id=fields.Many2one('account.account',string="Account",domain=_get_account_domain)
...
class Payment(models.Model):
    ...
    total_accounts_amount = fields.Float(
         compute='_get_total_accounts_amount', store=True)
    ...
    def _create_payment_entry(self, amount):
        ...
        if payment_type == 'payment_to_account' and not self.invoice_ids:
            ...
            for payment_account in payment_account_ids:
                ...
                debit, credit, amount_currency, currency_id = aml_obj.with_context(date=self.payment_date).compute_amount_fields(payment_account.amount, self.currency_id, self.company_id.currency_id, invoice_currency)
                total_debit += debit
                total_credit += credit
        ...
        if not self.currency_id.is_zero(self.amount):
            ...
            #raise ValidationError("Debit"+str(liquidity_aml_dict["debit"])+" Credit"+str(liquidity_aml_dict["credit"])+" Amount"+str(amount))
        ...
        return move
        #added account_id argument for the counterpart account to be passed
        #while looking through payment accounts
